...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if defaults.object(forKey: "first") == nil {
            ...
            print("Передача контролера модели в первый раз SD")
            
            window?.rootViewController = storybourd
            window?.makeKeyAndVisible()
        } else {
            if let MMVC = window?.rootViewController as? GameVC { MMVC.modelConroller = ModelConrolle() }
            print("Передача контролера модели SD ")
        }
